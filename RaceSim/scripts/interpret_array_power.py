"""
Given the result csv from a simulation, find the average and maximum of array power
"""
...
parser.add_argument('-a', '--array', type=str, default='Array_Power(W)', help='Array Power column name (default: Array_Power(W))')
...
def get_average_power(csv_path, array_col_name, datetime_col_name):
    ...
    while(end_idx < num_points-1):
        ...
        if (t1_before_t2(end_time_str, x[end_idx])):
            if (end_idx > start_idx and not negative_zone):
        ...
        if y[end_idx] < 0.0:
            if (not negative_zone):
                # Calculate area if entering negative area for the first time
                negative_zone = True
                first_datetime = x[start_idx]
                hours_from_start = np.array([(dt - first_datetime).total_seconds() / 3600 for dt in x[start_idx:end_idx]])
                total_area += np.trapz(y[start_idx:end_idx], hours_from_start)
...
if __name__ == "__main__":
    ...
    csv_path = args.path
    array_power_column_name = args.array
    time_column_name = args.time
    get_average_power(csv_path, array_power_column_name, time_column_name)
